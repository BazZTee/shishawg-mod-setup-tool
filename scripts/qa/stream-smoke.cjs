...
async function run() {
  ...
  await evaluate('showView("view-quickactions");');
  await check('All five areas survive the rebuild', 'document.querySelectorAll(".sa-workspace [data-glass-area]").length===5');
  await check('Original action controls remain connected', '["btn-qa-update-title","btn-qa-update-game","btn-qa-create-clip","btn-qa-start-raid","btn-qa-cancel-raid","qa-commands-grid","qa-yt-search-input"].every(id=>!!document.getElementById(id))');
  await check('Tabs switch between commands and videos', '(()=>{document.getElementById("sa-tab-1").click();const okay=!document.getElementById("sa-library-1").hidden && document.getElementById("sa-library-0").hidden;return okay;})()');
  await screenshot('chat-commands-tab');
  await check('Both disclosure sections open', '(()=>{const buttons=[...document.querySelectorAll(".sa-fold-trigger")];buttons.forEach(b=>b.click());return buttons.length===2 && buttons.every(b=>b.getAttribute("aria-expanded")==="true");})()');
  await check('Favorite click does not send a command', '(()=>{const star=document.querySelector(".sa-pin");if(!star)return false;star.click();return star.getAttribute("aria-pressed")==="true" && !!document.querySelector(".sa-favorites .sa-shortcuts button");})()');
  await screenshot('chat-commands-with-favorite');
  await evaluate('document.querySelectorAll(".sa-pin").forEach(s => { if (s.getAttribute("aria-pressed") !== "true") s.click(); });');
  await screenshot('many-favorites');
  assert.equal(calls.filter(c=>c.channel==='twitch:send-chat').length,0);
  assert.deepEqual(errors,[]);
  fs.writeFileSync(path.join(output,'result.json'),JSON.stringify({checks,errors}));win.destroy();app.quit();
}
run().catch(error=>{fs.writeFileSync(path.join(output,'failure.txt'),error.stack);app.exit(1);});
