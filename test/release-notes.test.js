...
test('release notes appear once for the matching unseen version', () => {
  const firstLaunch = resolveReleaseNotes('8.0.2', notes, '');
  assert.equal(firstLaunch.shouldShow, true);
  assert.equal(firstLaunch.release.version, '8.0.2');
  assert.ok(firstLaunch.release.items.length > 0);

  const alreadySeen = resolveReleaseNotes('8.0.2', notes, '8.0.2');
